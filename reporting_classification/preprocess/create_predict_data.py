...
def main(args):
    ...
    # Location of CSV files (folder)
    originals_csv_dir = os.path.join(DATA_DIR, 'predict_data', args.input_folder, 'originals')
    csv_files_list = []
    csv_files_num_tweets = []

    for csv_file in os.listdir(originals_csv_dir):
        if re.match(r'\b(\S+).csv\b', csv_file):
            csv_files_list.append(os.path.join(originals_csv_dir,csv_file))
            df = pd.read_csv(os.path.join(originals_csv_dir,csv_file), header=None)
            csv_files_num_tweets.append(len(df))

    # Create run dirs
    run_name = get_run_name(args)
    run_dir = os.path.join(DATA_DIR, 'predict_data', args.input_folder, run_name)
    preprocessed_folder = os.path.join(run_dir, 'preprocessed')
    tfrecord_folder = os.path.join(run_dir, 'tfrecords')
    ...
    res = parallel(process_fn_delayed(
        ...
        tfrecord_folder,
        preprocessed_folder,
        args) for f_name in tqdm(csv_files_list, desc='Processing input files'))

    data = dict(vars(args))
    # Add two list items in the dictionary
    data['original_csv_files'] = csv_files_list
    data['num_preprocessed_tweets'] = csv_files_num_tweets
...
def parse_args():
    ...
    parser.add_argument('--model_class', default='covid-twitter-bert', choices=PRETRAINED_MODELS.keys(), help='Model class')
    parser.add_argument('--input_folder', required=True, type=str, help='Name of the input CSV files to process')
    ...
    add_bool_arg(parser, 'asciify_emojis', default=True, help='Asciify emojis')
    ...
    add_bool_arg(parser, 'standardize_punctuation', default=True, help='Standardize (asciify) special punctuation')
