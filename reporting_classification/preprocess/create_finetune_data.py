...
def main(args):
    ...
    run_dir = os.path.join(DATA_DIR, 'finetune_multinomial', run_name)
    if not os.path.isdir(run_dir):
        os.makedirs(run_dir)
    # find input data
    originals_dir = os.path.join(DATA_DIR, 'finetune_multinomial', 'originals')
...
def parse_args():
    ...
    parser.add_argument('--finetune_datasets', type=str, nargs='+', help='Finetune dataset(s) to process. These correspond to folder names in data/finetune. \
            Data should be located in data/finetune_multinomial/originals/{finetune_dataset}/[train.tsv/dev.tsv/test.tsv]. By default runs all datasets.')
