...
from utils.cache_helpers import find_project_root
...
def notebook_plot(func):
    ...
    def wrapper(*args, **kwargs):
        # style_sheet = os.path.join(find_project_root(), 'utils','analysis_helpers', 'stylesheets', 'notebook.mplstyle')
        style_sheet = os.path.join('utils', 'analysis_helpers', 'stylesheets', 'notebook.mplstyle')
...
def paper_plot(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        # style_sheet = os.path.join(find_project_root(), 'utils', 'analysis_helpers', 'stylesheets', 'paper.mplstyle')
        style_sheet = os.path.join('utils', 'analysis_helpers', 'stylesheets', 'paper.mplstyle')
        plt.style.use(style_sheet)
        return func(*args, **kwargs)
    return wrapper

def use_stylesheet(name='paper'):
    # style_sheet = os.path.join(find_project_root(), 'utils', 'analysis_helpers', 'stylesheets', '{}.mplstyle'.format(name))
    style_sheet = os.path.join('utils','analysis_helpers', 'stylesheets', '{}.mplstyle'.format(name))
    plt.style.use(style_sheet)
...
def save_fig(fig, name, plot_formats=['png', 'eps'], version=1, dpi=300):
    ...
    folder_path = os.path.join('plots', name, 'v{}'.format(version))
